...
window.timestamp.holoplay = '2019/08/14 11:19:37'
/* eslint- disable */
var THREE, localStorage, msgfix, WebSocket, CustomEvent;

function HoloPlay(scened, camerad, rendererd, focalPointVector, constantCenter, hiResRender){
    //Version 0.2.3
    ...
    function init()
    {
        ...
        if(focalPointVector === undefined){
            ...
            camerad.getWorldDirection(vector); //Sets the vector to the camera forward

            viewScale = Math.max(camerad.position.length(), 1); //Sets the focal distance to either the distance to center or just ahead of the camera
            ...
            focalPointVector = [camerad.position.x + vector.x, camerad.position.y + vector.y, camerad.position.z + vector.z]; //Sets the focal point to the front of the camera as far away as it is from (0,0,0)
            ...
            viewScale = Math.max(center.distanceTo(camerad.position), 1) //Sets the focal distance to either the distance to center or just ahead of the camera
        }
        ...
        camerad.getWorldDirection(cameraForward);
        ...
        startNear = camerad.near;
        startFar = camerad.far;
        ...
        //const window = {innerWidth: rs.x, innerHeight: rs.y};
	...
	function applyCalibration (calibration_obj)
	{
        if(!calibration_obj){
            ...
            msgfix('holo', "No Looking Glass display connected; using default calibration data. Please ensure your Looking Glass is connected to your computer via USB and reload the page.")
        } else {
            jsonObj = JSON.parse(calibration_obj);
        }
		setShaderValues(jsonObj.DPI.value, jsonObj.pitch.value, jsonObj.slope.value, jsonObj.screenH.value, jsonObj.screenW.value, jsonObj.center.value, jsonObj.flipImageX.value, jsonObj.flipImageY.value, jsonObj.invView.value);  // sjpt patch
	...
	function doLoadEEPROM (inInit)
	{
		...
		ws.onerror = function(event) {
			if (!msgfix('holo', "Three.js driver not detected! Click OK to download. If you have already installed the driver, please make sure port 11222 is open.")){
    ...
    function setShaderValues(dpi, pitch, slope, screenH, screenW, centerp, flipX, flipY, invView)
    {
        ...
        bufferMat.uniforms.center.value = centerp;
    ...
    //compute the different views but do not render sjpt
    function computeViews (camera)
    {
        ...
        return arraycamera.cameras;
    }
    HoloPlay.prototype.computeViews = computeViews;

    // compute and render the views
    function captureViews(scene, camera) {
        computeViews(camera);
        rendererd.setRenderTarget(bufferSceneRender);
        rendererd.render(scene, arraycamera);
    };
    ...
    HoloPlay.prototype.render = function (scene, camera, renderer, preponly = false){
        if(!initialized)
            return false;
        ...
        if(!threeD){
            ...
            if(jsonObj == null){
                msgfix('holo', "No calibration found! Please ensure that your Looking Glass is plugged in.");
                return;
            }
    ...
    /** sjpt patch */
    this.interleave = function(source) {
        rendererd.setRenderTarget(null);
        bufferMat.uniforms.quiltTexture.value = source;
        //bufferMat.uniforms.flipX.value = Holo.flipX;
        //bufferMat.uniforms.flipY.value = Holo.flipY;
        rendererd.render(finalRenderScene, finalRenderCamera);
    }
