...
var geneSpeed: any = {}, geneSpeedSave = {};  // for animation
...
function applyDampedRotation()
{
    ...
    if (1*rotTarget.dx === rotTarget.dx) rotTarget.dx = 0;
    if (1*rotTarget.dy === rotTarget.dy) rotTarget.dy = 0;
...
var animStep = function animStepF(rev = inputs.doAnimRev, tomutate = genedefs, animSpeed = inputs.animSpeed) {
    ...
    if (sineAnimFrom) {
        ...
        if (rev) inputs.animSpeed *= -1;

        steerall();
        for (let gn in tomutate) {
            if (gn === 'gscale') continue;
...
if (!hoverSteerMode) {
                ...
                // eg by Kinect code
                if (v > gdef.max) v = gdef.max;
                if (v < gdef.min) v = gdef.min;
}
                setval(gn, v);
                }
            //currentGenes[gn] = v;
...
function rotNormal() {
    ...
    inputs.yzrot = -0.124; inputs.xzrot = 0.517;
        ...
        ****/
        ...
        if (renderObjHorn.centreOnDisplay) {
            ...
            if (targetAutocam) {
                ...
                if (!cc) cc = currentGenes._gcentre = new THREE.Vector4();
...
function steerall() {
    ...
    if (hoverSteerMode && isSteeringInteraction()) {  // sjpt we do NOT want this in main code till more debugging ....
        ...
        for(let gn in genedefs) {
            if (genedefs[gn].free === 0)
                {}//currentGenes[gn] = eo[gn];
        }
    }
    ...
    if (usesteer && pppt !== 0) {
        ...
        if (hoverSteerMode && useaverage) makeaverage();
    }
}
...
function makeaverage() {
    var pppt = 0;
    for (let o in currentObjects) if (currentObjects[o].genes) pppt += currentObjects[o].ppp;

    var genes = {};
    for (let gn in currentGenes) genes[gn] = 0;
    //var mmm = "";
    for (let o in currentObjects) {
        var dispobj = currentObjects[o];
        if (!dispobj.genes) continue;
...
function normalizeGeneSpeed() {
    ...
    for (let gn in geneSpeed) {
        if(!isNaN(geneSpeed[gn])) geneSpeed[gn] *= ss;
    }
}
...
function steer(steering, genes, level, vn) {
    ...
    if (dist < steerclose) return ppp;
    if (isNaN(ppp)) serious("bad ppp computed");
    if (ppp === 0 || isNaN(ppp)) return ppp;
    ...
    for (let gn in currentGenes) {
        ...
        if (isNaN(d)) {
            ...
            if (steering[gn] === undefined) steering[gn] = 0;
            steering[gn] += d * ppp;
...
async function animateNum(time: number, frame?) {
    ...
    if (_ininit) return; //
    inAnimate = true;
    if (inputs.doAnim || alwaysNewframe) myRequestAnimationFrame();  // do asap: set as nop() for XR
    if (alwaysNewframe) { if (alwaysNewframe as any === true) alwaysNewframe = 1e15; alwaysNewframe--; newmain(); }
    ...
    for (let i=0; i < 50; i++) {  // loop to help measure separated cpu time, only with testcputimes.running
        ...
        if (!testcputimes || !testcputimes.running)
            ...
            if (testcputimes && testcputimes.running === "time") consoleTimeEnd("animate_" + i);
    }
    inAnimate = true;
 ...
 * NOT USED FOR XR
 */
var myRequestAnimationFrame:any = function() {
...
function animatee(now) {
    ...
    if (logframetime === -1) logframetime = logframealltime = performance.now();
    ...
    if (framenum < 10) log('nextframe+++++++++++++++++++++++++++++++++++', framenum);
...
//        log(framenum, "framedeltasmooth", "framedelta");
    ...
    if (inputs.doAnim || springs.running) framescale();
    ...
    if (tracketclate) tracketc();
    if (logframenum >= framenum) console.table(logframetable);
    ...
    if (animate.readafter) readpixtest();
...
function modGeneSpeed(filter, modrate) {
    for (let gn in genedefs) {
        if (filter(genedefs[gn])) {
            ...
            if (old * geneSpeed[gn] < 0) geneSpeed[gn] *= -1;
        }
    }
}
...
function toDefault(genes) {
    if (genes === undefined) genes = target;
    for (let gn in genedefs) target[gn] = genedefs[gn].def;
    newmain();
}
...
function toTarget() {
    ...
    lasttargtime = tt;
    if (target === undefined || olength(target) === 0) return;
    ...
    if (done && !inputs.doAnim) framescale();
...
function sineAnimate(filter?: Genes) {
    ...
    W.doAnim.checked = true; saveInputState();
}
...
function fillCamGenes(genes) {
    ...
    if(genes._camx === undefined) genes._camx = camera.position.x;
    if(genes._camy === undefined ) genes._camy = camera.position.y;
    if(genes._camz === undefined ) genes._camz = camera.position.z;
    if(genes._camqx === undefined) genes._camqx = camera.quaternion.x;
    if(genes._camqy === undefined) genes._camqy = camera.quaternion.y;
    if(genes._camqz === undefined) genes._camqz = camera.quaternion.z;
    if(genes._camqw === undefined) genes._camqw = camera.quaternion.w;
    if (genes._fov === undefined) genes._fov = camera.fov;
}
