 ...
 */
COL.genes2col = function(genes, low=0, high=undefined, bounds: [RegExp, number, number?][] = COL.bounds) {
    const hset = getHornSet(genes);
    if (!hset) return;
    const gns = hset.colgenenames;
    if (!gns) return;  // temporary till we work out how to colour extra horn sets
    if (high === undefined) high = hset.hornrun.length - 1;
    const ulow = low * COL.PARMS, uhigh = (high+1) * COL.PARMS;
    if (bounds) for (const p of bounds) if (p[2] === undefined) p[2] = p[1];
...
export function setWallColours(genes: Genes) {
    for (let gn in COL.num) {
        const wgn = 'wall_' + gn;
        if (!(wgn in genes) && wgn in genedefs) genes[wgn] = genedefs[wgn].def;
...
export function setHornColours(genes: Genes) {
    if (tad && genes.tranrule.startsWith('//tadpoleTranrule')) {
        setWallColours(genes);
        return tad.useCOL(); //
    }
    const hset = getHornSet(genes);
    if (COL.ignoreHornColours === 'usewall') {COL.genes2col(genes, 2, 2)}; // set just wall colours
    if (COL.ignoreHornColours && hset.colgenenames) return;
    ...
    if (hset.colgenenames) {
        ...
        hset.colgenenames = [];
        for (let gn in COL.num) {
            if (!(gn in genes)) genes[gn] = genedefs[gn].def;
...
const KLEFT = '<';
const KRIGHT = '>';
type HornChildType = {subname: string, start?:NS, end?: NS, num?: NS, depth?: NS, cage?: boolean}; // for sub, head, tail
...
type ProcessTranOpt = [] | [string, OptType, N,N,N,N,string,string?, NS?, string?];
type Subtype = '_sub' | '_head' | '_tail' | '_none';

type _parseret = {pretranrule, posttranrule, mainhorn, luniforms, overrides, endoverrides, extraIncludes} & {error}
...
type NS = N | string;
export type OptType = NS | OptStruct;
 ...
 */
export function toJava(o): string {
    if ( typeof o === 'string') return "'" + (o as Xstring).replace(/'/g, "\\'") + "'";
    if ( typeof o === 'number') return o.toString();
    const r = [];
    for (const x in o)
        if (o[x] !== undefined) r.push(x + ': ' + toJava(o[x]));
    return '{' + r.join(', ') + '}';
}
...
export class OptStruct {
    ...
    toString() {
        ...
        for (const x of 'k,start,end,v,horn,rand,randp,randb,randc,randx'.split(','))
            if (this[x] !== undefined) r.push(x + ': ' + toJava(this[x]));
        return '{' + r.join(', ') + '}';
    }
}
...
class Tran {
    ...
    opts: OptType[];
    _genecode: string[];
    constructor(name: string, opts: OptType[]) {
        this.name = name;
        this.opts = opts;
    }

    html(genes:Genes, hset:HornSet, name:string, i:number) {
...
export class Horn {
    ...
    _ribs: number = 10;
    _ribdepth: number = 1;
    _rref: number|string = undefined;
    _cols: any[]=[];
    hset: HornSet;
    _usescale:boolean = true;
    _genenames: {} = {};
    trancode: string;
    _genecode: string;
    _coluse: Dictionary<string>;
    hascage: boolean;
    _radius: number| string;

    // lots below that will be automatically generated
    stack: (...a)=>Horn;
    bend: (...a)=>Horn;
    curl: (...a)=>Horn;
    swapn: (...a)=>Horn;
    ...
    constructor(name, hset) {
        this.name = name;
        this.rp = this.gn('rp');
        if (hset) hset.horns[name] = this;
        this.hset = hset;
        // this._usescale = true;
    }

    /** return gene name in standard format, allowing for possible recursion */
    gn(gn: string, lev?: N) { return this.name + "_" + (lev ? lev + "_" : "") + gn; };
    ...
    _addgene(fun:string, def:number, min:number, max:number, delta:number, step:number, help:string, tag:string, free:number | string = 1,
        useuniform:boolean = true, genes: Genes) {
    ...
    */
    _expr(fun: string, s: OptType, min:number, max:number, delta:number, step:number, help:string, tag:string='geom', free: number | string=1, genes: Genes) {
        if (typeof s === "number") {
    ...
    */
    ...
    processTran(code: string, tran: Tran, defs: ProcessTranOpt[], opts: Dictionary<any>, genes: Genes) {
        const userparms = tran.opts;
        for (const i in userparms) if (typeof userparms[i] === 'object') userparms[i] = new OptStruct(userparms[i]);
        ...
        for (let i = 0; i < defs.length; i++) {      // merge specified arguments with default arguments
            ...
            let defval = defvala;
            if (typeof defval === 'object') def[1] = defval = new OptStruct(defval);
            let s = userparms[i];
            ...
            if (sq.rand !== undefined) {
                xp = this._expr(fun + "_R", sq.rand, (min??0)/10, (max??1)/10, (delta??0.1)/10, (step??0.1)/10, help + ' random me based', tag, free, genes);
                const xpp = `(randf(ribrp(rp,dribs),${rr()})*${xp})`;
                repl.push(xpp);
                if (toshow) genecodel.push("rand:" + this._show(xp));
            }
            if (sq.randb !== undefined) {
                xp = this._expr(fun + "_RB", sq.randb, (min??0)/10, (max??1)/10, (delta??0.1)/10, (step??0.1)/10, help + ' random me/parent based', tag, free, genes);
                const xpp = `((randf(SUBP_rp - ribrp(rp,dribs),${rr()}))*${xp})`;
                repl.push(xpp);
                if (toshow) genecodel.push("randb:" + this._show(xp));
            }
            if (sq.randp !== undefined) {
                xp = this._expr(fun + "_RP", sq.randp, (min??0)/10, (max??1)/10, (delta??0.1)/10, (step??0.1)/10, help + ' random parent based', tag, free, genes);
                const xpp = `(randf(SUBP_rp,${rr()})*${xp})`;
                repl.push(xpp);
                if (toshow) genecodel.push("randp:" + this._show(xp));
            }
            if (sq.randc !== undefined) {
                xp = this._expr(fun + "_RC", sq.randc, (min??0)/10, (max??1)/10, (delta??0.1)/10, (step??0.1)/10, help + ' random cumulative', tag, free, genes);
                const xpp = `(randf(crp,${rr()})*${xp})`;
                repl.push(xpp);
                if (toshow) genecodel.push("randc:" + this._show(xp));
            }
            if (sq.randx !== undefined) {
                xp = this._expr(fun + "_RX", sq.randx, (min??0)/10, (max??1)/10, (delta??0.1)/10, (step??0.1)/10, help + ' random xyz', tag, free, genes);
                const xpp = `(randf(x,${rr('x')})*${xp} + randf(y,${rr('y')})*${xp} + randf(z,${rr('z')})*${xp})`;
                repl.push(xpp);
                if (toshow) genecodel.push("randx:" + this._show(xp));
            }
            // if (sq.vvv !== undefined) {
		...
		// but we are unlikely? to want multiple brahces from same point so don't bother for now.
                ...
                if (lastsub?.num !== undefined) {
                    ...
                    if (this._genenames[this.gn("radius")]) msgfixerrorlog('radius() in' + this.name, 'probably not used')
                } else {
    ...
    _addgeneXX(fun:string, def:number, min:number, max:number, delta:number, step:number, help:string, tag:string, free:number | string = 1,
        useuniform:boolean = true) {
    ...
    wig(low:N, high:N, timefreq:N, lenfreq:N) {
        const tr = this.trans[this.trans.length - 1];
        // const oldv = tr.opts[0];
        const oname = tr.name + '_';

        if (!this._genenames) this._genenames = {}; // [] if needed but wrong place
    ...
    /** It finds the previous tranform, pixks up the first parameter, and replaces it with equivalent wig call */
    pulse(low:N, high:N, timefreq:N, lenfreq:N, pulsewidth:N) {
        const tr = this.trans[this.trans.length - 1];
        // const oldv = tr.opts[0];
        const oname = tr.name + '_';
    ...
    $stack(o: Horn, genes: Genes, s:Tran):Horn { return o.processTran("stack(MM0)", s, [["stack", 1000, 0, 2000, u, 0.1, "stack for horn"]], {}, genes); };
    $stackx(o: Horn, genes: Genes, s:Tran):Horn { return o.processTran("stackx(MM0)", s, [["stackx", 1000, 0, 2000, u, 0.1, "stack for horn"]], {}, genes); };
    $stacky(o: Horn, genes: Genes, s:Tran):Horn { return o.processTran("stacky(MM0)", s, [["stacky", 1000, 0, 2000, u, 0.1, "stack for horn"]], {}, genes); };
    $stackz(o: Horn, genes: Genes, s:Tran):Horn { return o.processTran("stackz(MM0)", s, [["stackz", 1000, 0, 2000, u, 0.1, "stack for horn"]], {}, genes); };
    $stackxyz(o: Horn, genes: Genes, s:Tran):Horn { return o.processTran("stackxyz(MM0, MM1, MM2)", s, [
            ["x", 1000, 0, 2000, u, 0.1, "x horn"],
            ["y", 1000, 0, 2000, u, 0.1, "y horn"],
            ["z", 1000, 0, 2000, u, 0.1, "z horn"]
        ], {}, genes);
    };
    $code(o: Horn, genes: Genes, s:Tran):Horn {
        const aa = [[]] as ProcessTranOpt[];
    ...
    $dynop(o: Horn, genes: Genes, s:Tran):Horn { return o.processTran("dynop(MM0)", s, [["dynop", {k:1}, 0, 20, u, 1, "dynamic op", "geom", "fixed", "k"]], {}, genes); };
    ...
    $gcode(o: Horn, genes: Genes, s:Tran):Horn {
    ...
    $bend(o: Horn, genes: Genes, s:Tran):Horn {
        if (s.opts.length <2) {
    ...
    $wiggle(o: Horn, genes: Genes, s:Tran):Horn {
        if (s.opts.length <3) {
    ...
    $curl(o: Horn, genes: Genes, s:Tran):Horn {
        if (s.opts.length <2) {
    ...
    $pintexture(o: Horn, genes: Genes, s:Tran):Horn { return o.processTran("pintexture(MM0)", s, [["pintexture", 1000, 0, 2000, u, 0.1, "nominal stack for texture"]], {}, genes); };
    $clelia(o: Horn, genes: Genes, s:Tran):Horn {
    ...
    $clelia2(o: Horn, genes: Genes, s:Tran):Horn {
    ...
    $twist(o: Horn, genes: Genes, s:Tran):Horn {
        if (s.opts.length <3) {
    ...
    $twax(o: Horn, genes: Genes, s:Tran):Horn {
    ...
    $spoke(o: Horn, genes: Genes, s:Tran):Horn {
    ...
    $spiral(o: Horn, genes: Genes, s:Tran):Horn {
    ...
    $swapn(o: Horn, genes: Genes, s:Tran):Horn { return o.processTran("swapn(LL0,LL1)", s, [[],[]], {funname: "swapn"}, genes); };
    $radiate(o: Horn, genes: Genes, s) {
    ...
    $tw4(o: Horn, genes: Genes, s:Tran):Horn  { return o.processTran("twr(LL0,LL1, MM2, MM3);", s, [
            [],
            [],
    ...
    $st4(o: Horn, genes: Genes, s:Tran):Horn { return o.processTran("st(LL0, MM1)", s, [[], ["st4", 1000, 0, 2000, u, 0.1, "stack for horn, 4d"]], {}, genes); };
    $growpart(o: Horn, genes: Genes, s) { return o.processTran("growpart(MM0,!RP,PARENT_rp,PARENT_rref,PARENT_ribs)", s, [
            ["growpart", {k: 0.1}, 0, 1, u, 0.01, "proportion at end with reduced growth", "geom", "fixed", "k"]
        ], {}, genes); };
    $growpartr(o: Horn, genes: Genes, s) { return o.processTran("growpartr(MM0,!RP,PARENT_rp,PARENT_rref,PARENT_ribs)", s, [
            ["growpartr", {k:0.1}, 0, 1, u, 0.01, "proportion at end with reduced radius", "geom", "fixed", "k"]
        ], {}, genes); };
    $web(o: Horn, genes: Genes, s) {
    ...
    _addtrlow(fun: string, args: OptType[]) {
    ...
    sub(h:string, num?, start?, end?, depth?) {
	...
	// corrected 16 Dec 2023, to remove Han 2024
        ...
        if (hs.depth !== undefined)
            hs.depth += ''; // force it to be a fixed value; matcodes gets confused otherwise???
        else
            hs.depth = '2';
        this._sub.push(hs);
        return this;
    ...
    tail(h) {
        if (HornSet.subsfortailsouter) this.sub(h, '0.', '1.', '1.');
        else if (HornSet.tailsToHead) this._head.push(h);
    ...
    head(h) {
        if (HornSet.subsfortailsouter) this.sub(h, '0.', '0.', '0.');
    ...
    uselist(list: HornChildType[], parents: string[], subparents: string[], type: Subtype, hset: HornSet, genes:Genes, medepth) {
        ...
        var ok = [];
        for (let i in list) {
            var item: HornChildType = list[i];

            let name: string = type === "_sub" ? item.subname : item as any as string;
            if (false) {
    ...
    //            console.log("ignoring recursive " + type + " " + name + " in " + this.name);
                ...
                if (type === "_sub") {
                    if (parents.slice(0, -1).indexOf(this.name) === -1) {
                        // var xx=['"'+name+'"'];
                        var xx = [];
                        if (item.num !== u) xx.push("num: " + this._show(this._expr("S_" + name + "_num", item.num, 0, 50, 1, 0.1, "number of instances of " + name + " under " + this.name,'geom','free',genes)));
                        if (item.start !== u) xx.push("start: " + this._show(this._expr("S_" + name + "_start", item.start, 0, 1, 0.1, 0.1, "start pos of " + name + " under " + this.name,'geom','free',genes)));
                        if (item.end !== u) xx.push("end: " + this._show(this._expr("S_" + name + "_end", item.end, 0, 1, 0.1, 0.1, "end pos of " + name + " under " + this.name,'geom','free',genes)));
                        if (item.depth !== u) xx.push("depth: " + this._show(this._expr("S_" + name + "_depth", item.depth, 0, 7, 1, 1, "recursion depth of " + name + " under " + this.name,'geom','free',genes)));
                ...
                } else throwe("unexpected type to this.uselist " + type);
    ...
    _compilecols(hset: HornSet, lev, genes:Genes) {
        if (_testcompile) {
        ...
        for (let name of COL.names) this._coluse[name] = name;
    ...
    **/
    ...
    generates items .trancode, ._genenames, _genecode
        ...
        **/
    _compileh1(parents: string[], subparents: string[], hset: HornSet, genes:Genes) {
       const ddd = this.login( '_compileh1');
        ...
        this._addgene('cutoffset', 0, -4000, 4000, 1, 1, 'cut offset for this horn, 999999 means uncut', 'geom', 'frozen', true, currentGenes);
    ...
    _colset(genes, uniformsp, lev, hornid) {
         if (tad && genes.tranrule.startsWith('//tadpoleTranrule')) return;
        var coluse;
        /**
    ...
    */
    _resolve(spec, role) {
        ...
        if (typeof spec === "string" && !isNaN(+spec)) return +spec;  // constant number "15"
    ...
    logout(d:number, ...a) { logdepth = d; this.hlog('<<', ...a); }

    /** render this horn, used for setting up, but not every frame, see rendersinglpemulti (was _renderme) */
    _compileh2b(genes: Genes, uniformsp, rendertarget, hset: HornSet, key: string, num: number, ribs, hornid: number, parentHornid: number, subtype: Subtype) {
        if (badshader) return;
       const ddd = this.login( '_renderme', opmode);
        ...
        if (rad || inputs.SINGLEMULTI) {  // zero or undefined will not show this object, but needs to be compiled for SINGLEMULTI
            ...
            if (uniformsp.light0s) uniformsp.light0s.value = genes.light0s * ((hornid === hset.hornhighlight) ? 5 : 1);
            ...
            (scene.children[0] as any).material.side = THREE.FrontSide;  // NO scene was the wrong object to hit in most cases
            ...
            if (_testcompile) {
                ...
                (sc.children[0] as any).material.wireframe = HW.usewireframe;
    ...
    was _rendersubs */
    _compilesubs(genes: Genes, uniformsp, rendertarget, hset: HornSet, key: string, num: number, ribs, hornid: number, parentHornid: number, subtype: Subtype) {
       const ddd = this.login( '_rendersubs');
        ...
        for (let ssubtype in {_sub: 0, _head: 0, _tail: 0}) {
            const subtype: Subtype = ssubtype as Subtype;
            if (!HornSet.subsfortails && subtype !== '_sub') continue;
            for (let i in this[subtype]) {
                var item = this[subtype][i];
                if (subtype === '_head') item = { subname: item, start: 0, end: 0, num: 0 }
                if (subtype === '_tail') item = { subname: item, start: 1, end: 1, num: 0 }
                ...
                if (sribs === undefined) sribs = item.num;
                if (sribs === undefined) sribs = ribs;
                if (item.num < 0) sribs = -item.num*ribs;
                ...
                for (let refl = 0; refl < (item.cage ? 2 : 1); refl++) {
                    ...
                    if (oldrender) {
                        ...
                        for (let k = sribs; k > -1; k--) {
                            ...
                            uniformsp.k.value = k;
                            horns[item.subname]._compileh2a(genes, uniformsp, rendertarget, hset, key, num, hornid, subtype);
                        }
                    } else {
                        ...
                        horns[item.subname]._compileh2a(genes, uniformsp, rendertarget, hset, subkey, subnum, hornid, subtype);
                        }
                }   // refl
                if (this.hascage) uniformsp[this.gn("reflx", medepth)].value = 1;
    ...
    _postcompilesub(uniformsp, rendertarget, hset: HornSet, key:string, num:number, h, refl, parentHornid) {
       const ddd = this.login( '_postcompilesub');
        ...
        if (startgn in hset.geneDefaults) var start = startgn;
        else if (h.start !== u) start = h.start;
        else start = '0.';
        if (endgn in hset.geneDefaults) var end = endgn;
        ...
        if ((sribs as any)%1 === 0) sribs = `float(${sribs})`;
     ...
     * num is the cumulative (product) number of instances of this
     was renderh */
    _compileh2a(genes: Genes, uniformsp, rendertarget, hset: HornSet, key: string, num: number, parentHornid: number, subtype: Subtype) {
       const ddd = this.login('_compileh2a')
        ...
        if (!HornSet.subsfortails) {        // we must do the heads and tails the old (pre April 2019) way
            for (i in this._tail) {
                ...
                horns[this._tail[i]]._compileh2a(genes, uniformsp, rendertarget, hset, subkey, num, hornid, '_tail'); // ??? 19/11/2022 parentHornid);
            }
            for (i in this._head) {
                uniformsp[this.gn("active", medepth)].value = 0;
                ...
                horns[this._head[i]]._compileh2a(genes, uniformsp, rendertarget, hset, subkey, num, hornid, '_head');
            }
        }
        /***/
    ...
    public static newHornStack = 400;
    public static newHornRadius = 80;
    public static newHornTwist = 770;
    public static newHornTwistOffset = 70;

    _newh(name, hset: HornSet, genes: Genes, recurse: boolean) {
    ...
    _addsub(hset: HornSet, genes: Genes, recurse: boolean) {
        const nn = this._newh(this.name + 'S', hset, genes, recurse);
        this.sub(nn, rr()*40);
    };
    /** add a tail for struct mutation or explicit */
    _addtail(hset: HornSet, genes: Genes, recurse: boolean) {
    ...
    _addtran(tran: Tran) {
        if (!tran._genecode) tran._genecode = [`#${this.name}_${tran.name}${this.trans.length}`]
    ...
    _tos(t: HornChildType, f: string): string {
        if (f === 'sub' && t.cage) f = 'ribcage';
        let s = `.${f}('${t.subname}'`;
        ...
        s += ')';
        for (let i=0; i<5; i++) s = s.replace(', undefined)', ')'); // remove all unneeded trailing undefined
        return s;
    }
    ...
    toString() {
       ...
       if (this._radius) s += `.radius(${toJava(this._radius)})`;
       if (this._ribs) s += `.ribs(${toJava(this._ribs)})`;
       if (this._ribdepth) s += `.ribdepth(${toJava(this._ribdepth)})`;
    ...
    mutateStructH(hset: HornSet, genes: Genes, recurse: boolean) {
        var r = Math.random();
        if (this.trans.length < 10) {
            if (rr() < 0.1) this._addtran(new Tran("twist", [360, new OptStruct({k:200})]));
            if (rr() < 0.1) this._addtran(new Tran("bend", [90]));
            if (rr() < 0.1) this._addtran(new Tran("curl", [90]));
            if (rr() < 0.1) this._addtran(new Tran("stack",[1000]));
            if (rr() < 0.1) this._addtran(new Tran("scale", [2]));
        }
        var hornn = Object.keys(hset.horns).length;
        if (recurse && hornn < HORNSTYPES) {
            if (rr() < 0.4) this._addsub(hset, genes, false);  // add subhorn
            if (rr() < 0.1) this._addtail(hset, genes,  false);  // add tail
        }
        // todo addhead
        ...
        if (genes[rn] < 10) genes[rn] = 10;         // don't hide things
    };
    ...
    /** make copy of main definition part of horn, ready for mutation, save  */
    // structCopy(hset) {
    //     let {name, rp, trans, _sub, _tail, _head, _ribs, _cols, _usescale, _radius} = this;
    //     const h = new Horn(name, hset);
    //     if (_cols.length === 0) _cols = [undefined];    //
    //     Object.assign(h, {name, rp, trans, _sub, _tail, _head, _ribs, _cols, _usescale, _radius});
    //     return h;
    // }

    //!!WA.getHornSet = getHornSet;


};  // end Horn

function getHornSet(genes: Genes):HornSet {
    if (!genes) return undefined;
    const tranrule = genes.tranrule; // .pre('SynthBus');
    if (tranrule === 'matrix') return undefined;
...
function xxxhset(xxx): HornSet {
    if (xxx instanceof HornSet) return xxx;
    const genes = xxxgenes(xxx);
    return getHornSet(genes);
}
function showtran(x) {
    const hset = xxxhset(x);
    console.table('###~~~', hset.toString() as any);
}
...
export class HornSet {
    static current(): HornSet { return currentHset; }

    horns: Dictionary<Horn> = {}
    //var trancode;
    varyings = "";  // code varyings for rp values etc <<< TODO DEAD??
    setupcode = "";  // code for setting up rp values etc
    pickoutput = "";  // code for returning pick information
    //segnames;   // names for horn segments
    trancode = "";  // accumulated transform code
    pretranrule = "";  // code to insert just before the tranrule
    _genecode = "";  // accumlated code showing gene names
    uniforms = "";  // accumlated code for uniforms
    addedUniforms = {} as Dictionary<boolean>;  // list of uniforms already added
    _springMap = undefined;  // set to true or array for automated spring mapping
    _gcode = "";    // global code, eg for extra variables
    _saveposset = {};  // set of savepos done, with true if not yet resolved
    gbuffoffsets = [];     // keep track of skelbuffer/skeleton offsets after set by OPMAKESKELBUFF
    skelnum = [];        // keep track of skelbuffer/skeleton resolutions after set by OPMAKESKELBUFF
    // horn so lo = undefined;       //name of so lo horn; now use hornhighlight && keysdown.includes('shift')
    hornhighlight: number = undefined;  // name of highlight hoen
    // so lo starttime = undefined; // start time of selection of so lo

    hornrule: string;
    header: string;
    mainh: string;
    geneDefaults: Dictionary<number>;
    // addedUniforms: Dictionary<boolean>;
    mainhorn: string;
    segnames: (string|number)[];
    code: string[];
    parentcode: string[];
    parents: number[];
    sribs: (string|number)[];
    ...
    trankey: string;
    bundle: HornSet;
    singlePassCode: string;
    chooseHornCode: string;
    mytotnum: number[];
    cumcount: number[];
    // ribs: number[];
    defribs: number[];  // # ribs defined eg in genes, may be non-integer
    horncount: number;
    fullHornCount: number;
    getSynthGeneNames: ()=>void;
    _renderscene: THREE.Scene;
    ribshow;
    meshused;
    gbuffoffset;
    lastgbuffoffset;
    parentComplexity: number;   // # hornruns in the parent

    colgenenames?;

    synthCode: string;
    static tailsToHead = false;
    static subsfortailsouter = true;  // <<<<<<<<<<< bad bug where proper head/tail not working so changed to true. 6 Nov 2023
    static subsfortails = false;
    static orderbug = false;
    wallgenes;
...
// orderbug = true will explicitly readd that bug for backward compatibility
    ...
    /*^^^async*/ parsehorn(tranrule, genes: Genes, testing: boolean = false) : _parseret {
        if (!_testcompile) log('parsing horn', tranrule.substring(0,40));
        /** called from within parsehorn */
        function tocompile(genes: Genes, hset: HornSet) : _parseret {
            var ugene = gene; // for historic usage
            var pretranrule='', posttranrule='', luniforms = {}, overrides='', endoverrides='', extraIncludes='', mainhorn=undefined;
            nop('$$$');
            return {pretranrule, posttranrule, mainhorn, luniforms, overrides, endoverrides, extraIncludes} as _parseret;

            // so the tranrule can define genes if needed
            function gene(name:string, def:N, min:N, max:N, delta:N, step:N, help:string, tag:string, free:NS, useuniform=true) {
            ...
            function _pulse(name) {
                ...
                var p = "(pulserate, pulseperhorn, pulsepow, pulsescale, pulsemodrate, pulsemodscale, pulsemax, pulseendfade, crp, r, xscale, rp, time)"
            ...
            function pulsermult(name) {
                const rr = 'r = ' + _pulse(name) + ';\n';
                // log('pulser output', rr);
                posttranrule += rr; // for posttranrule implicit in tranrule
                return rr;          // for posttranrule = ... explicity in tranrule (backward compatability)
            }
            ...
            function cut(pnames, perHornOffset = false) {
                const u = undefined;
                let names = pnames;
                if (typeof pnames === 'string') names = [names];
                else if (typeof pnames === 'number') names = new Array(names).fill(0).map((x,i)=>'cut'+i);
        ...
        try {
            ...
            try {
                ...
                if (testing) return {error: e} as _parseret;
        ...
        var mainh: string = answer.mainhorn;
        ...
        if ((mainh as any) instanceof Horn) { serious('??? parse returned Horn'); mainh = (mainh as any).name; }
    ...
    _addgeneHS(gn:string, def:number, min:number, max:number, delta:number, step:number, help:string, tag:string, free:number|string,
            useuniform:boolean = true, genes:Genes ) {
    ...
    */
    _adduniformX(gn, def, type='f', forceuniform:boolean = false, useuniform:boolean = true, gv:N = def) { // REMOVED 15/07/2021, forceuniform:boolean = false) {
        if (this.addedUniforms[gn]) return;
    ...
    static specgenes = {};
    //######### todo use genes
    _addgenespec(gn:string, s, min:N, max:N, delta:N, step:N, help:string, tag:string, free: N|string, genes:Genes) {
        this._addgeneHS(gn, s, min, max, delta, step, help, tag, free, undefined, genes);
        HornSet.specgenes[gn] = true;
    };

    // compile hornset. For some reason this can't be called twice at the same time, even for different hsets
    static _incompilehs;
    _compilehs(tranrule: string, genes: Genes) {
        if (HornSet._incompilehs) serious('recursive call to _compilehs')
    ...
    _compilehsI(tranrule: string, genes: Genes) {
        logdepth = 0;
...
//        this.xxxtranrule = skelReplace(genes.tranrule, this.horns);
        ...
        this.parentcode = [];
        this.parents = [];
        this.sribs = []; this.sribs[3] = 0; // name for subcount gene
        //this.ribs = [];                     // #subhorns
     ...
     */
    _genhornrun(genes: Genes, uniformsp) {
        var cumcount = 0;
        ...
        for (let h = 3; h < this.hornrun.length; h++) {
            ...
            if (isNaN(+sribs)) {                 // sribs is a gene anme
                nsribs = genes[sribs];
                if (nsribs < 0) nsribs = genes[sribs] = 0;
            ...
            if (this.hornhighlight) {
                if (kkk === 'Delete' && h === this.hornhighlight) show = false;          // <<< hide
                else if (kkk === 'Insert' && h !== this.hornhighlight) show = false;     // <<< solo
                else if (h == this.hornhighlight) {                                         /// << highlight
                    var ccc = (frametime/1000 % 1 > 0.5) ? 'red' : 'green';
                    for (let b = 1; b <= 3; b++) COL.setG(ccc + b, h, 5, u, this);
                }
            }
            hr.start = cumcount;
            if (show)
        ...
        if (cumcount > inputs.MAXHORNS_TO_RENDER) cumcount = inputs.MAXHORNS_TO_RENDER; // n/a to CSynth, but works ok
        this.horncount = cumcount;
        if (uniformsp) uniformsp.horncount.value = cumcount;
        return cumcount;
    }
    ...
    rendersinglemulti(genes: Genes, uniformsp, rendertarget: THREE.WebGLRenderTarget, scenep: THREE.Scene) {
        if (_testcompile) return true;
        ...
        if (cumcount === 0) return true;


        if (!scenep) {
            // use standard multiscene but cheat the skelnum and gbuffoffset for now
            if (!HW.multiScenedummy) HW.multiScenedummy = { hornrun: { nohorn:{} }, trankey: '', gbuffoffsets: {}, skelnum: {} };
            ...
            if (!scenep.children[0]) {log('cannot rendersinglemulti: no scene children for ' /*?? + kkopmode */, opmode, oplist[opmode]); return false; }
        ...
        const mat = (scenep.children[0] as any).material;
        mat.wireframe = HW.usewireframe && ((rendertarget as any).name === 'rtopos');
        if (usemask === -99) {mat.wireframe = opmode === 0; mat.needsUpdate = true; }
    ...
    _reducePath(n, targ, genes = currentGenes) {
        ...
        for (let g of gg) target[g.gn] = genes[g.gn] * reduce;
    }

    /** reduce total number of horns based on given factor */
    static reduceHorns(dispobj:Dispobj = xxxdispobj(lastTouchedDispobj), fac = 0.8, genes = xxxgenes(dispobj)) {
    ...
    static reduceHornsToLimit(dispobj:Dispobj = xxxdispobj(lastTouchedDispobj), fac = 0.8, genes = xxxgenes(dispobj), limit = inputs.MAXHORNS_TO_RENDER) {
        genes = xxxgenes(genes);
        const hset = xxxhset(dispobj);
        let i = 0;
        hset._genhornrun(genes, uniforms);
    ...
    static controlMutation(bool: boolean) {
    ...
    mutateStructHs(genes) {
        ...
        try {
            ...
            for (let h in this.horns) nhs.horns[h].mutateStructH(nhs, genes, true);    // use this.horns to get horn names, nhs.horns will be mutated during this loop
    ...
    structCopy(): HornSet {
        ...
        for (const h in nhs.horns) nhs.horns[h].hset = nhs;
    ...
    _hornname(b: string) {
        if (!this.horns[b]) return b;
        for (let i=2; i < 10; i++) {
            const nn = b+i;
            if (!this.horns[nn]) {
                this.horns[nn] = 'pending' as any;
                return nn;
            }
        }
    }
            ...
            for (const sub of horn._sub) {
                if (sub.num === undefined) {
                    ...
                    if (gd) {
                        ...
                        addgene(sn, gd.def, gd.min,gd.max, gd.delta, gd.step,
                            `number of ${sub.subname} as subhorns of ${horn.name}`, gd.tag, gd.free);
                        genes[sn] = sub.num;                        // keep current value of num
                        if (gd.max === 200) gd.max = 1000;          // now sensible to allow many more ribs
                    }
                }
            }
...
WA.mutateStructHs = function(genes: Genes, fromdo: Dispobj, todo: Dispobj) {
    const sgenes:Genes = {}; copyFrom(sgenes, genes);
    const hset = getHornSet(genes);
    const nhs = hset.mutateStructHs(sgenes);
    sgenes.tranrule = nhs.tranrule;
    // console.log('~~smutate~~~', fromdo.vn, todo.vn, '\n', genes.tranrule, '\n', sgenes.tranrule);
    // console.table('~~~~~~~~~~~~~~~~~~\n' + nhs.toString());
    function col(n) {    // colour when the genes are available, which isn't until nhs is rendered
        if (n <= 0) return;
        if (nhs.colgenenames) {
            COL.randcols2(); COL.col2genes(sgenes);
        } else {
            onframe(() => col(n-1));
        }
        todo.needsRender = 2;
...
function hornTrancodeForTranrule(tranrulea:string, genes:Genes, recurse: boolean = true):HornSet {
...
function horn(name: string, hset: HornSet):Horn {
    hset = hset || _defaultHornSet;
    if (hset.horns[name] && hset.horns[name] !== 'pending' as any) return hset.horns[name];
...
var _hornSets: Dictionary<HornSet> = {};
/** add to directory of built hornsets */
function setHornSet(key:string=undefined, val:HornSet=undefined) {
    if (key === undefined) {     //
        _hornSets = {};
    } else {
        _hornSets[key] = val;
...
function renderHornobj(genes:Genes, uniformsp, rendertarget, scenep, hset = getHornSet(genes)) {
    ...
    if (!done) {
        hset.getmain()._compileh2a(genes, uniformsp, rendertarget, hset, "", 1, -999, '_none');
 ...
 */
 function randrulemarry(genes: Genes, obj2) {
    var newrule = randrules(genes);
    trancodeForTranrule(newrule, genes);  // make sure rule is established
    var newgenes:Genes = clone(genes);
    newgenes.tranrule = newrule;
    if (genes.tranrule.indexOf('horn') === -1) return newgenes;
 ...
 */
function multiScene(genes: Genes, num: number, key: string, dummy, hset: HornSet, hornid: number | string) : THREE.Scene {
    ...
    if (inputs.USESKELBUFFER && opmode === OPMAKESKELBUFF) {
        ...
        if (HW.resoverride.skelnum) skelnum = HW.resoverride.skelnum;
    ...
    if (!dummy) {
        ...
        var bigscene: BigsceneType = HW.bigsceneSet[rb];
        if (!bigscene) {
            ...
            if (dradnum > 0) {
                let uarr: Uint32Array | Uint16Array;
            ...
            HW.bigsceneSet[rb] = bigscene as any;
        ...
        if (!bscene.children[0]) {
            ...
            mesh.frustumCulled = false;
            (bscene as any).addX(mesh);
        }

        var mat = getMaterial(genes.tranrule, genes);
        (bscene.children[0] as any).material = mat;
...
function multiInstances(bigscene: BigsceneType, num: number) {
    var geometry = bigscene.geometry;
    var ageometry = geometry as any;
    ...
    if (ises300) {
        ...
        if (ageometry._maxInstanceCount === undefined) ageometry._maxInstanceCount = Infinity;
    ...
    if (HW.instanceids.length < num) {
        for (let i = HW.instanceids.length; i < num * 1.3 + 50; i++) HW.instanceids.push(i);
...
function updateHTMLRules(genes) {
    if (!W.htmlouter) return;   // for CSynth, odd module loading order
...
function trkeydown(evt) {
    ...
    todo = false;
    switch (ff) {
       case "ArrowUp": incgene(gn, k, evt); break;
       case "ArrowDown": incgene(gn, -k, evt); break;
       case "Enter":
            saveundo();
...
function trannamekeydown(evt) {
    ...
    var todo = false;
    switch (ff) {
       case "ArrowUp": newops("move up"); break;
       case "ArrowDown": newops("move down"); break;
       default: todo = true;
   }
    if (!todo) return killev(evt);
    return true;
    ...
    if (op === "delete") {
        ...
        if (nhs.horns[nname]) {
            for (let n=0; true; n++) {
                ...
                if (!nhs.horns[nname + n]) break; // <<<<<<<<<<<<<<< WRONG, we need to check if a horn exists, nothing to do with getHornSet> ???nhs.horns[nname]???
            }
        }
...
function ppreframe() {
    ...
    if (!genes.P_ribs) return;
    var scurve = function(x) { return (3-2*x)*x*x; };
    var t = 120*1000;  // overall time cycle
    var swt = 3*1000; // swap time
    var k = (frametime % t * 2) / t;
    var sw = 2*swt/t;
    if (k < sw) genes.P_ribs = 1 + scurve(k/sw);
    else if (k < 1) genes.P_ribs = 2;
    else if (k < 1 + sw) genes.P_ribs = 2 - scurve((k-1)/sw);
    else genes.P_ribs = 1.0000001;
    scalehalflife = 1500;
}
...
function colourTailor(hornid, genes) { // return;
    var p = hornid;
    ...
    if (inputs.FLUORESC) {
        ...
        // frametime in msec
        // G.time in secs
        var usetime = (genes._fixtime ?? genes.time ?? frametime/1000)*1000; // genes.time set in setObjUniforms
...
function setGenesFromTranruleDEAD(ptranrule /* = currentGenes.tranrule */, trankey=undefined) {
    ...
    try {
        if (!trankey) trankey = trancodeForTranrule(ptranrule, currentGenes).trankey;
 ...
 */
function codeForUniforms(hset: HornSet, genes: Genes) {
...
async function saveSkeleton(fid, dispobj = lastTouchedDispobj ?? xxxdispobj()) {
    ...
    if (fid && fid !== '!temp') writetextremote(fid, skelj);
...
readtest.vvx = []; readtest.vvy = []; readtest.vvz = []; readtest.vvw = [];
...
export const HW = {
    ...
    newplane: true,
    dotty: false,  // set to true to generate dotty scene: also used for skelbuffer generation
    ...
    bigsceneSet: {} as Dictionary<BigsceneType>,  // cached scene set big enough for everything seen so far: one bigscene for each fac
    bigsceneSet_lastokkey: '',
    instanceids: [],   // buffer for HW.instanceids, not needed in webgl2
    ...
    radnums: [3,4,5,7,9,13,17,25,33,49,65,91,127,177,260],  // HW.radnums used for varying resolution, values above 65 or so give bugs

    resoverride: {} as {lennum?: number, radnum?: number, skelnum?: number, skelends?: number},
