...
class CSynthAnnotation {
    static nextGroupID: number = 0;
    constructor(text: string, iOrAnnot: number | AnnotDesc | any, g = CSynth.annotationGroup) {
    }
}

type AnnotDesc = {
    "Associated Gene Name": string, "Gene Start (bp)": number, "Gene End (bp)": number
    minI: number,
    maxI: number,
    midI: number,
}
...
(function () {
    ...
    CSynth.createAnnotation = function(text, iOrAnnot, g = CSynth.annotationGroup) {
        ...
        if (g) g.color = CSynth.annotationGroup ? CSynth.annotationGroup.color : new THREE.Color(1, 1, 1);
        if (!g) {
            ...
            let setupSpringPasses = function setupSpringPasses(originalIndices, linkData) {
                ...
                const posCopyUniforms = {
                    posNewvals: W.uniforms.posNewvals, //global W version of posNewvals
                ...
                const destIndAttrib = posCopyGeom.attributes.destIndex as THREE.BufferAttribute;
                ...
                if (g.linkMesh) g.remove(g.linkMesh);
                g.linkMesh = linkMesh;
                g.add(linkMesh);
                ...
                springs.onPostSpringStep(() => {
                    ...
                    if (!g.visible || !g === CSynth.annotationGroup) return;
            ...
            g.initSprings = (len=1, str=1, pow=1, fgui) => {
                ...
                g.createDebugGUI = () => { ///TODO --> help text / default on ~ minTexScale
                    ...
                    Object.entries(g.springs.uniforms).forEach(e => {
                        const name = e[0], u = e[1] as any;
                        if (u.min === undefined) return;
                ...
                if (fgui) { // at least have spring strength in the gui
                    ...
                    u.springforce.value = 1; u.pushapartforce.value = 0;
                    o.spread = 0.4;
                ...
                const linkData = annotations.map((annot, li) => {
                    ...
                    if (slot === undefined) log(`bad annotation spring v=${oldV} part=${li} annot=${annotj}`);
            ...
            g.filter = (textFilter) => {
                ...
                if (g.userData.unfilteredAnnotations) g.userData.unmergedAnnotations = g.userData.unfilteredAnnotations;
...
CSynth.geometry23d = function(geometry) {
    ...
    if (!pos) {log('geometry no position attribute'); return; }
    const isize = pos.itemSize;

    if (isize === 3) return;
    if (isize !== 2) {console.error('cannot make geometry 3d, unexpected position attribute', isize); return; }
    const oa = pos.array;
    const na = new oa.constructor(oa.length * 3 / 2);
    let j = 0; for (let i=0; i < oa.length; i += 2) {na[j++] = oa[i]; na[j++] = oa[i+1]; na[j++] = 0; }
    geometry.setAttribute('position', new THREE.BufferAttribute(na, 3));
    log('geometry corrected from 2d to 3d position', geometry.name ?? 'unnamed')
}
