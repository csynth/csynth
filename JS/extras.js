...
var resoverride, skelbuffer, getcentrescale, readWebGlFloatDirect, remotesave;
async function genwinsom(skn = 10) {
    resoverride.skelnum = skn
    resoverride.skelends = 0
    skelbuffer = 0;

    await S.frame(4);

    var sc = getcentrescale();
    var ss = readWebGlFloatDirect(skelbuffer, {w: U.skelnum, h: currentHset.horncount});
    var o = [];
    o.push('size(10);')
    o.push('view righthand window(-1,1,-1,1,-1,1) normal(0, 0, -1) up(0, 1, 0) ref(0, 0, 0) eyedist(5);')

    o.push('xx = empty union (empty union (empty')
    let bad = 0, ok = 0;
    for (let i=0; i<ss.length; ) {
        ...
        const z = ss[i++];
        const r = ss[i++];
        if (x < sc.lx || x > sc.hx || y < sc.ly || y > sc.hy || z < sc.lz || z > sc.zx)
            {bad++; continue; } // ???
        if (r > 0) {
            ok++;
            o.push(`union sphere(${r.toFixed(3)}) at(${x.toFixed(3)}, ${y.toFixed(3)}, ${z.toFixed(3)})`)
            // o.push('{}')
            // o.push(`union sphere(0) at(0,0,0)`)
            if (ok%100 === 0)
                o.push(') union (empty ');
            if (ok%10000 === 0)
                o.push(') union (empty {' + ok + '}');
        }
    }
    o.push('))');

    log('winsom spheres seen', ok, 'expected', currentHset.horncount * resoverride.skelnum, 'bad', bad);


    o.push(`; draw xx at(${-sc.x}, ${-sc.y}, ${-sc.z}) scale(${2/sc.wmax});.`)
    var str = o.join(' ')
    var name = 'test' + resoverride.skelnum
    remotesave(name + '.winsom', str)

    runcommandphp('start "winsom" cmd /c ..\\winsom\\winsom.cmd . ' + name)
}