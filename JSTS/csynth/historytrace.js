...
`;
    ...
    Maestro.on("preframe", () => {
        ...
        if (keysdown.join('') === 'Q')
            htuniforms.highlightTime.value = 1 - lastdocx / width;
...
function OrganicHistoryTrace() {
    if (springs.getHISTLEN() === 0)
        springs.setHISTLEN(32);
    ...
    // tad.topos();
    if (!V.gui)
        V.gui = dat.GUIVR.createX("forhistorytrace");
    V.gui.addFolder(VH.hist.createGUIVR());
}
