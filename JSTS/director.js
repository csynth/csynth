...
var Director = new function () {
    ...
    D.setup = function () {
        ...
        if (D.inbetween === D.keyframesInbetween) {
            if (slots.length === 1) {
                msgfixlog("Cannot animate with only one object", "try alt-1 and mutate to open some viewports");
                return;
            }
            renderObjHorn.centreOnDisplay = false;
    ...
    D.setAnimlen = function () {
        ...
        if (+W.animlen.value === 0.23)
            setInput(W.animlen, 0.23333333333); // for keyframes and every 1/2 way frame
        ...
        if (Director.inbetween !== Director.keyframesInbetween)
            return; // no speed/len tie except with keyframes
    ...
    D.framesFromSlots = function () {
        ...
        for (let i = 1; i <= D.slotsToUse; i++) {
            if (!slots[i])
                return log("framesFromSlots called with no slot.", i);
        }
        ...
        for (var i = 1; i <= D.slotsUsed; i++) {
            if (slots[i] && i !== mainvp && slots[i].x !== 9999) {
                ...
                fgenes._uXrot = _uXrot;
                fgenes._uYrot = _uYrot;
                fgenes._uZrot = _uZrot;
                if (i === 1)
    ...
    D.keyframesInbetween = function D_keyframesInbetween(p2, genelist, genes = currentGenes) {
        ...
        framepos += fp2;
        p2 = p2 - fp2;
        if (framepos + 3 > frames.length) {
            framepos = frames.length - 3;
            p2 = 1;
        } // so spline gets values it can use
        if (framepos < 1) {
            framepos = 1;
            p2 = 0;
        }
        ...
        {
            let secs = Math.floor(animtime - (D.useoffset ? inputs.animsstart : 0));
        ...
        for (var gn in glist) {
            if ((genedefs[gn]) || (gn.startsWith('_') && typeof glist[gn] === 'number')) {
                ...
                if (D.splinetype === 0)
                    gv = p2 * fgenes[2][gn] + p1 * fgenes[1][gn]; // linear
    ...
    D.setframeauto = function D_setframeauto() {
        if (curtime >= D.renderEnd / inputs.animlen)
            curtime = D.renderEnd / inputs.animlen;
        if (curtime <= D.renderStart / inputs.animlen)
            curtime = D.renderStart / inputs.animlen;
        var delta = (frametime - lastframetime) * 0.001; // delta time in secs
        if (lastframetime <= 0)
            delta = 0; // for just starting up, establish initial lastframetime but don't move
    ...
    D.setframe = function D_setframe(vtime, genelist) {
        ...
        if (vtime > 1 && vtime - 1 < 1e-8)
            vtime = 1;
        curtime = vtime;
        if (vtime > 1 || vtime > D.renderEnd / inputs.animlen) {
            if (V.gui)
                V.gui.visible = true;
            return false;
        }
        ...
        if (D.inbetween === D.keyframesInbetween) { // for keyframe director
            if (isNaN(framepos))
                framepos = 0;
            if (framepos + 1 >= frames.length || curtime > frames[framepos + 1].time)
                framepos = 0;
            while (frames[framepos + 1].time <= curtime + 1e-10)
                framepos++; // 4 to use are fp-1, fp, fp+1, fp+2
    ...
    D.plot = function (gn) {
        if (frames.length === 0) {
            msgfix('no Director set up', '?');
            return;
        }
        log("plotting", gn);
        var saveshow = inputs.showhtmlrules;
        inputs.showhtmlrules = false;
        var savegene = currentGenes[gn];
        if (D.vals)
            for (var ff = 1; ff < frames.length; ff++) {
                frames[ff].genes[gn] = D.vals[ff - 1];
            }
        for (let i = 0; i < frames.length; i++)
            frames[i].genes = frames[i].zzgenes; // slots[i].dispobj.genes;
     ...
     */
    D.keyGenes = function keyGenes(pattern, ...values) {
        const filt = resolveFilter(pattern);
        if (typeof values[0] === 'function')
            values = new Array(Director.slotsToUse).map((v, i) => values[0](i));
        if (values.length === 1)
            values = new Array(Director.slotsToUse).fill(values[0]);
