...
function valuecode(genes = G) {
    ...
    if (!t1)
        return genes.tranrule; // in case called before compile
    for (var gn in genes) {
...
function orginit() {
    ...
    try {
        ...
        if (!WA.isCSynth)
            makeDraggable(W.msgbox, { usesize: false });
...
var guiboxgenes = { notyetset: true }; // use to keep track of last seen genes in the gui
...
function _CodeMirrorInstanceOnChange() {
    ...
    if (olength(genes))
        guiboxgenes = genes; // indirect in case optimization prevents sensible return of genes
    if (typeof res === 'string') {
        ...
        msgfix('compile', 'OK');
        if (inps.useinterp)
            G.tranrule = code; // immediate update as we can
    }
...
function initCodeMirror() {
    ...
    _CodeMirrorInstance.setOption("extraKeys", { "Ctrl-Space": "autocomplete",
        ...
        "Shift-Alt-R": function () {
            ...
            if (s.width === '99%') {
                s.width = s.left = '';
                WA.cm_tranrule.style.maxWidth = '1200px';
            }
            else {
                s.width = '99%';
                s.left = '0';
                WA.cm_tranrule.style.maxWidth = '';
            }
        },
        "Ctrl-.": function (cm) {
...
function tranruleFromGenes(genes = currentGenes, tranrule = currentGenes.tranrule) {
    ...
    try {
        ...
        for (let i = 1; i < ss.length; i += 2)
            if (!isNaN(ss[i]))
                ss[i] = i + kk; // replace potential gene values with markers in ss
...
function setTranruleFromGenes(genes = currentGenes, tranrule = _CodeMirrorInstance.getValue()) {
    const sg = currentGenes;
    currentGenes = undefined; // just make sure we are not playing with currentGenes by mistake
    try {
