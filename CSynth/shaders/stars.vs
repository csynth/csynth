/*
Initially copied from https://threejs.org/examples/webgl_gpu_particle_system.html
Likely to end up quite different.
*/
...
void main() {
    ...
    if( timeElapsed > 0. ) {
        gl_Position = logdepth(projectionMatrix * modelViewMatrix * vec4( newPosition, 1.0 ));
    } else {
        gl_Position = logdepth(projectionMatrix * modelViewMatrix * vec4( position, 1.0 ));
        lifeLeft = 0.;
        gl_PointSize = 0.;
    }
}
