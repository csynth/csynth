...
//     genes for t0 etc
//     iGlobalTime defined
// This version has been cleaned up to remove some experimental bits such as 'trap'
...
#define recursions int(Recursions)
		...
		const int	RECURSIONS 	= 10;  // << MAX recursions
		...
		float map(in vec3 p, out bool isHit) {
			vec4 zz = vec4( p, 0.2 );
			float mod = 0.0;
			float dzz = 1.0;
...
if (i >= recursions) break;
		...
		float intersect( in vec3 ro, in vec3 rd, out bool isHit ) {
			float maxd = 50.0;
		...
		vec3 calcNormal( in vec3 pos, float e ) {
			...
			return normalize( vec3(
				   map(pos+eps.xyy, hit) - map(pos-eps.xyy, hit),
				   map(pos+eps.yxy, hit) - map(pos-eps.yxy, hit),
				   map(pos+eps.yyx, hit) - map(pos-eps.yyx, hit) ) );
		}
		...
		vec3 fractProgram( in vec2 p, in float time )
		{
			...
			// raymarch
			bool isHit; 
			float dist = intersect(ro,rd, isHit);
